...
연구실에 우분투 AI 훈련 고성능 PC가 있어서 yolo model 학습을 진행하였다.
학습 데이터는 실제로 넘어지는 모습을 촬영한 동영상 데이터 5개와 AI HUB 에서 제공하는 싸우는 모습의 cctv 데이터들을 가져와서 라벨링을 진행하였고, roboflow에서 증강 기능을 활용하여 1226장의 훈련데이터를 확보했다. yolo model의 result_matrix를 확인한 결과 precision와 recall이 가장 좋은 결과를 보여준 파라미터 값은 epochs = 500 , patience = 0 , batch = 8 imgsz = (1440,2560) 였다.
